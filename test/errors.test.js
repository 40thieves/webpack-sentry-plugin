...
it('adds error if Sentry organisation slug is missing', () => {
  return runWebpack(createWebpackConfig({ organisation: null })).catch(({
    errors,
  }) => {
...
it('adds error if Sentry project name is missing', () => {
  return runWebpack(createWebpackConfig({ project: null })).catch(({
    errors,
  }) => {
    expect(errors).toHaveLength(1);
...
it('adds error if Sentry api key is missing', () => {
  return runWebpack(createWebpackConfig({ apiKey: null })).catch(({
    errors,
  }) => {
    expect(errors).toHaveLength(1);
...
it('adds release error to compilation', () => {
  return runWebpack(createWebpackConfig({ release: 'bad-release' })).catch(({
    errors,
  }) => {
    expect(errors).toHaveLength(1);
...
it('adds upload error to compilation', () => {
  return runWebpack(createWebpackConfig({ release: 'bad-upload' })).catch(({
    errors,
  }) => {
    expect(errors).toHaveLength(1);
