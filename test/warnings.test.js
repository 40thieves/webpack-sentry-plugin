...
it('adds warning if Sentry organisation slug is missing', () => {
  return runWebpack(createWebpackConfig({ organisation: null, suppressErrors: true })).catch((
    { warnings },
  ) => {
...
it('adds warning if Sentry project name is missing', () => {
  return runWebpack(createWebpackConfig({ project: null, suppressErrors: true })).catch((
    { warnings },
  ) => {
    expect(warnings).toHaveLength(1);
...
it('adds warning if Sentry api key is missing', () => {
  return runWebpack(createWebpackConfig({ apiKey: null, suppressErrors: true })).catch((
    { warnings },
  ) => {
    expect(warnings).toHaveLength(1);
...
it('adds warning if release version is missing', () => {
  return runWebpack(createWebpackConfig({ suppressErrors: true })).catch(({ warnings }) => {
    expect(warnings).toHaveLength(1);
...
it('adds release warning to compilation', () => {
  return runWebpack(createWebpackConfig({ release: 'bad-release', suppressErrors: true })).catch((
    { warnings },
  ) => {
    expect(warnings).toHaveLength(1);
...
it('adds upload warning to compilation', () => {
  return runWebpack(createWebpackConfig({ release: 'bad-upload', suppressErrors: true })).catch((
    { warnings },
  ) => {
    expect(warnings).toHaveLength(1);
